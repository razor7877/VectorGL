...
{
	double generationTime = Timing::measureTime([&]() {
		...
		this->generateIBL(renderer);
	});
	char buf[32];
	std::snprintf(buf, sizeof(buf), "%.2f", generationTime * 1000);

	Logger::logInfo("HDR IBL generation took " + std::string(buf) + " ms", "IBLData.cpp");
}

IBLData::IBLData(Renderer& renderer, std::unique_ptr<Cubemap> cubemap) : environmentMap(std::move(cubemap))
{
	double generationTime = Timing::measureTime([&](){this->generateIBL(renderer);});
	char buf[32];
	std::snprintf(buf, sizeof(buf), "%.2f", generationTime * 1000);

	Logger::logInfo("IBL generation took " + std::string(buf) + " ms", "IBLData.cpp");
}

void IBLData::generateIBL(Renderer& renderer)
{
